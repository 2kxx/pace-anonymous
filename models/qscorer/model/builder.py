...
from models.qscorer.model import *
...
def load_pretrained_model(
    ...
    if "qscorer" in model_name.lower():
        # Load LLaVA model
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            tokenizer = LlamaTokenizer.from_pretrained(preprocessor_path, use_fast=False)
            print("Loading mPLUG-Owl2 from base model...")
            model = MPLUGOwl2LlamaForCausalLM.from_pretrained(
                model_base, low_cpu_mem_usage=True, config=lora_cfg_pretrained, **kwargs
            )

            token_list = ['<score1>', '<score2>', '<score3>', '<score4>', '<score5>']
            num_new_tokens = tokenizer.add_tokens(token_list, special_tokens=True)
            score_token_id = tokenizer.convert_tokens_to_ids(token_list)
            score_token_id.reverse()
            model.score_token_id = score_token_id
            if num_new_tokens > 0:
